...
If you're interested in a full _professional history_, as they say, you can find it right here:
...
export async function getStaticProps() {
  const records = await fetchAllRecords("Currently");
  const mungedRecords = records.map((r) => {
    return {
      verb: r.fields.Verb,
      title: r.fields["Name (from Content)"][0],
    };
  });
  fs.writeFileSync(`./public/currently.json`, JSON.stringify(mungedRecords));
  return {
    props: {
      items: mungedRecords,
    },
